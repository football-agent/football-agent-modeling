...
df <- shuffled_data[, -which(names(shuffled_data) %in% c("X", "log_value_eur"))]
...
for (col in colnames(df)) {
  ...
  if (col %in% c("id", "squad", "league", "player", "squad", "contract_signing", "elite_team")){
    print("skip")
  }else if (startsWith(col, "Team_")) {
    team_dummy_features <- c(team_dummy_features, col)
  }
  else if(col %in% c("xg_net", "npxg_net", "xa_net")){
    print("skip because log 0")
...
for (col in colnames(df)) {
  if (startsWith(col, "Team_")) {
    team_features = c(team_features, col)  
    }else if (col %in% c("id", "player", "squad", "contract_signing", "elite_team", "value_eur")){
    print("skip")
  }else if (col %in% c("standing", "team_avg" ,"team_std", "rich_team")){
    team_features <- c(team_features, col)
  }else if (startsWith(col, "SEASON.")) {
    season_features <- c(season_features, col)
  }else {
    player_features = c(player_features, col)
  }
}

player_data = df[c("player", player_features)]
team_data = unique(df[c(team_features, team_dummy_features)])
season_data = unique(df[c(season_features)])

df = df[c(player_features, team_features, team_dummy_features, "value_eur")]
...
set.seed(1234)
indices <- sample(1:nrow(df), size = 0.9 * nrow(df))
train <- m[indices,]
test <- m[-indices,]
...
# )
hyper_grid <- expand.grid(max_depth = seq(3,5,2),
                          eta = seq(.05, .25, .1),
                          gamma = seq(0.1, 0.7, 0.2),
                          subsample = seq(0.5, 0.9, 0.2),
                          colsample_bytree = seq(0.1, 0.7, 0.2)
)
xgb_train_rmse <- NULL
xgb_test_rmse <- NULL
...
plot(y,
     main = "Scatter plot of predicted value versus ground truth",
     yhat,
     xlab = "actual",
     ylab = "predicted")
abline(lm(yhat ~ y))
...
#####################################
# for (col in names(transform_log)){
#   transformation = transform_log[col]
#   if (transformation == "log"){
#     full_data[col] = log(full_data[col]+1)
#   }else if(transformation == "sqrt"){
#     full_data[col] = sqrt(full_data[col])
#     }
# }


grid=merge(player_data, team_data,by=NULL);

predict_data_ = grid[c(player_features, team_features, team_dummy_features)]
predict_data = grid[c("player")]

d <- data.frame(predict_data_)
m <- as.matrix(d)
...
grid_result <- predict(m1_xgb, m)

# team_short <- team_long[team_long$value == 1,]
# > grid=merge(player_data, team_short,by=NULL);
# > grid=merge(grid, season_data,by=NULL);
# > 
#   > grid$prediction = grid_prediction
# Fehler: Objekt 'grid_prediction' nicht gefunden
# > grid$prediction = grid_result
# > View(grid)
# > result = grid[c("player", "Team", season_features, "prediction")]
# > result$prediction =  exp(result$prediction)
# > write.csv(result, "result.csv")

team_long <- pivot_longer(team_data, names_to = "Team", cols = starts_with("Team_"))
team_short <- team_long[team_long$value == 1,]

result_df=merge(player_data, team_short,by=NULL)[c("player", "Team")]
result_df$prediction = grid_result
result_df$Team = gsub("\\."," ",result_df$Team)
result_df$Team = sub("Team_", "", result_df$Team)
result_df$prediction = exp(result_df$prediction)
